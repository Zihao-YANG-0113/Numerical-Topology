...
def _fmt_matching(matching) -> str:
    if not matching:
        return "{}"
    return "{" + ", ".join(
        f"({format_simplex(a)} ◁ {format_simplex(b)})" for a, b in matching
    ) + "}"


def _fmt_critical(critical) -> str:
    if not critical:
        return "{}"
    return "{" + ", ".join(format_simplex(c) for c in critical) + "}"
...
def main():
    ...
    for entry in report["examples"]:
        ...
        md.append(f"- Matching: {_fmt_matching(entry['matching'])}")
        md.append(f"- Critical: {_fmt_critical(entry['critical'])}")
    ...
    print(f"Wrote: {OUT / 'gallery_report.json'}")
    print(f"Wrote: {OUT / 'gallery_report.md'}")
