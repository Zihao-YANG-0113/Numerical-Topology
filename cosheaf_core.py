...
def format_simplex(s) -> str:
    """Render a simplex (tuple or list of vertex labels) compactly:
    - ('a',)       -> 'a'        (('a','b') -> 'ab')  for single-char string labels
    - (0,)         -> '0'        ((0,1) -> '(0,1)')   for integer/other labels
    """
    if len(s) == 1:
        x = s[0]
        return x if isinstance(x, str) else str(x)
    if all(isinstance(x, str) and len(x) == 1 for x in s):
        return "".join(s)
...
def verify_matching_acyclic(matching: List[Tuple[Simplex, Simplex]]) -> bool:
    """Check that the partial matching has no Sigma-cycle (Def 3.2, Prop 5.1).

    Builds the DAG whose nodes are matched pairs (sigma_i, tau_i), with an
    edge (sigma_a, tau_a) -> (sigma_b, tau_b) iff tau_a >| sigma_b, i.e.
    sigma_b is a codim-1 face of tau_a distinct from sigma_a. Acyclicity is
    checked by Kahn's topological sort: if all nodes are reachable in
    topological order, the matching graph has no directed cycle.
    """
    n = len(matching)
    sigma_to_idx = {sigma: i for i, (sigma, _) in enumerate(matching)}
    adj: Dict[int, List[int]] = {i: [] for i in range(n)}
    in_deg = [0] * n
    for i, (sigma_a, tau_a) in enumerate(matching):
        for face in codim1_faces(tau_a):
            if face == sigma_a:
                continue
            j = sigma_to_idx.get(face)
            if j is not None:
                adj[i].append(j)
                in_deg[j] += 1
